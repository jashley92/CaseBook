...
public sealed partial class ReportGenerator : IReportGenerator
{
    ...
    public byte[] GenerateWord(CaseReportModel m)
    {
        ...
        using (var doc = WordprocessingDocument.Create(ms, WordprocessingDocumentType.Document, true))
        {
            ...
            body.AppendChild(P(
                $"Generated by {m.GeneratedBy} at {m.GeneratedAtUtc:u}. {m.ContentHashLabel}: {m.ContentHash}",
                italic: true, size: 16));
    ...
    /// <summary>
    /// The separate lessons-learned report body (E-26/PROD-41): case identity, the post-incident review, and
    /// the improvement actions. Deliberately nothing else from the case record.
    /// </summary>
    private static void AppendLessons(Body body, CaseReportModel m)
    {
        ...
        if (m.ImprovementActions.Count == 0)
            body.AppendChild(P(m.NoActionsText));
        else
            body.AppendChild(WordTable(
                ["Improvement action", "Related area", "Details", "Owner", "Target (UTC)", "Status"],
